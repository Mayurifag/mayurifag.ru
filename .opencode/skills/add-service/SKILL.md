...
Required repo changes most likely will be:

- Create `roles/<service>/defaults/main.yml` and `roles/<service>/tasks/main.yml`; add `templates/` and other folders only when needed.
...
- Add `tinyauth@docker` for private services without proper ldap configuration.

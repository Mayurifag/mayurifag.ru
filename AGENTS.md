...
Current production preference is `STORAGE_USERS_POSIX_PROPAGATOR=sync`; async wrote `/storage/changes/*` inside the watched
root and caused watcher churn/log spam.

Do not delete `/var/lib/opencloud/nats` or disable NATS KV persistence for OpenCloud POSIX storage during normal deploys.
Use `opencloud_reset_state=true` only for a deliberate fresh-state repair.
